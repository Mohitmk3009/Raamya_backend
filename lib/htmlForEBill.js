...
const getHtmlForEBill = (order) => {
    ...
    const { address, street, city, state, postalCode, country, phone } = order.shippingAddress || {};

    // Build address lines conditionally, so we only include parts that exist.
    const addressLine1 = [address, street].filter(Boolean).join(', ');
    const cityState = [city, state].filter(Boolean).join(', ');
    const addressLine2 = [cityState, postalCode].filter(Boolean).join(' - ');
    ...
    <body>
        <div class="invoice-box">
            ...
            <div class="order-details-summary">
                ...
                <div style="display: flex; gap:5px;">
                    <strong>Date:</strong> <span>${formatTime(order.createdAt)}</span>
                </div>
